 ...
 * Run with: ./build/src/ui_ppo 0.0.0.0 8000
...
class websocket_session : public std::enable_shared_from_this<websocket_session> {
    ...
    struct CONFIG: learning_to_fly::config::ppo::PPOConfig<ABLATION_SPEC> {
        ...
        static constexpr TI STEP_LIMIT = 10001;
...
public:
    ...
    void on_read(beast::error_code ec, std::size_t bytes_transferred) {
        ...
        if(message["channel"] == "startTraining") {
            std::cout << "startTraining message received (PPO)" << std::endl;
            start(10);
            ...
            this->t = std::thread([seed, this]() {
                ...
                learning_to_fly::ppo_training::init(this->ts, seed);
                for(TI step_i=0; step_i < CONFIG::STEP_LIMIT; step_i++) {
                    learning_to_fly::ppo_training::step(this->ts);
                }
                training_end = std::chrono::high_resolution_clock::now();
                std::cout << "PPO Training took " << (std::chrono::duration_cast<std::chrono::milliseconds>(training_end - training_start).count())/1000 << "s" << std::endl;
            });
            t.detach();
        }
...
int main(int argc, char* argv[]) {
    std::cout << "========================================" << std::endl;
    std::cout << "Learning to Fly - PPO Training with UI" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << "Note: Run from the main repo directory" << std::endl;
    std::cout << "Example: ./build/src/ui_ppo 0.0.0.0 8000" << std::endl;
    std::cout << std::endl;
    
    try {
        if(argc != 3) {
            std::cerr << "Usage: " << argv[0] << " <address> <port>\n";
            std::cerr << "Example: " << argv[0] << " 0.0.0.0 8000\n";
            return EXIT_FAILURE;
        }
        ...
        std::cout << "Web interface available at: http://" << address << ":" << port << std::endl;
        std::cout << "Open this URL in your browser, then click 'Start Training'" << std::endl;
